...
Для того, чтобы убедиться в том, что трафик ходит по заданному маршруту можно использовать ```tcpdump``` ожидая ICMP или UDP пакеты в зависимости от того какие инструменты трассировки и диагностики используются. Пример: ```tcpdump -i eth1 icmp```.
