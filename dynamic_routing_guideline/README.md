...
Полную информацию об этих параметрах можно получить [тут](https://www.kernel.org/doc/Documentation/networking/ip-sysctl.txt). Нам же потребуется перевести параметры sysctl в соответствие со следующей таблицей:  
...
Т.к. пакет quagga входит в стандартный набор пакетов CentOS 7.8, то установка производится простым набором команды ```yum install quagga -y```. Однако следует иметь ввиду некоторые сопуствующие детали:
...
Аналогичная настройка производится на других роутерах автономной системы, но без команды ```default-information originate```, т.к. именно она распространяет маршрут по умолчанию через роутер R1 и указывает на точку выхода из автономной системы. Не забудьте сохранить конфигурацию!

Полное описание всех команды можно найти на [официальном ресурсе}(https://www.quagga.net/docs.html). Пройдем по приведённым выше командам.  
...
- ```network 10.0.0.1/32 area 0``` - указываем сеть, информация о которой будет рассылаться в автономной системе OSPF и принадлежность этой сети к конкретной зоне.

### Настройка опций OSPF на интерфейсах

В процессе настройки асимметричной маршрутизации понадобится изменить интерфейс-специфичные опции, такие как "цена интерфейса". Делается это в режиме конфигурирования интерфейса:
```
interface eth1
 ip ospf cost 1000
```
...
Для того, чтобы убедится в том, что трафик ходит по заданному маршруту можно использовать ```tcpdump``` ожидая ICMP или UDP пакеты в зависимости от того какие инструменты трассировки и диагностики используются. Пример: ```tcpdump -i eth1 icmp```.
