...
Т.к. пакет quagga входит в стандартный набор пакетов CentOS 7, то установка производится простым набором команды ```yum install quagga -y```. Однако следует иметь ввиду некоторые сопутствующие детали:
...
В режиме ```conf t``` есть множество команд. Нам для успешного выполнения работы потребуются 2 режима внутри ```conf t```: режим настройки интерфейсов и режим настройки процесса OSPF.  
